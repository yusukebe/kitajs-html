...
/**
 * Checks if the error is a timeout error thrown by the ErrorBoundary's
 * `timeout` property.
 *
...
export function isTimeoutError(
  error: unknown
): error is Error & { message: 'timeout' }
...
export interface ErrorBoundaryProps {
   ...
   *
   * The error will be string `timeout` if the rejection was caused by
   * the `timeout` property.
   */
  catch: JSX.Element | ((error: unknown) => JSX.Element)

  /**
   * If we should use the catch error boundary if the children takes longer
   * than the timeout. Use `undefined` or `0` to disable the timeout.	
   */
  timeout?: number
}
